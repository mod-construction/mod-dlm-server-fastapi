...
class PrefabElementPerformanceFireResistance(BaseModel):
    ...
    def reaction_to_fire_classification_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('A1', 'A2', 'B', 'C', 'D', 'E', 'F',):
            raise ValueError("must be one of enum values ('A1', 'A2', 'B', 'C', 'D', 'E', 'F')")
        return value

    @field_validator('fire_resistance_duration')
    def fire_resistance_duration_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('30min', '60min', '90min', '120min', '180min', '240min',):
            raise ValueError("must be one of enum values ('30min', '60min', '90min', '120min', '180min', '240min')")
        return value
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of PrefabElementPerformanceFireResistance from a JSON string"""
    ...
    def from_dict(cls, obj: Dict) -> Self:
        """Create an instance of PrefabElementPerformanceFireResistance from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "reactionToFireClassification": obj.get("reactionToFireClassification"),
            "fireResistanceDuration": obj.get("fireResistanceDuration")
        })
        return _obj
